...
To upload symbols to Backtrace, you need to rename symbols generated by Unity. By default, each symbol will end with extension `.sym.so`. Backtrace will successfully analyse symbols that ends with `.so` extension instead. To upload symbols please go to your project settings, to the `Upload an archive` tab under `Symbols` section. 
