...
/// Set to <code>0</code> for unlimited submissions. Default <code>30</code>.
@property (nonatomic) NSInteger reportsPerMin;
/// Logging destinations installed before repository and crash reporter startup.
/// Leave <code>nil</code> to preserve destinations installed through <code>BacktraceLogger.setDestinations(_:)</code>.
/// Set an empty collection to explicitly disable logging.
@property (nonatomic, copy) NSSet<BacktraceBaseDestination *> * _Nullable loggingDestinations;
...
@property (nonatomic, weak) id <BacktraceClientDelegate> _Nullable delegate;
...
///
/// \param reportsPerMin Maximum records sent in 1 minute.
/// Set to <code>0</code> for unlimited submissions. Default: <code>30</code>.
///
...
@property (nonatomic, readonly) BOOL handlerInstallationAttempted;
...
/// Maximum physical database size in MB. <code>0</code> means “no limit”. Default <code>0</code>.
...
/// \param loggingDestinations Logging destinations.
///
+ (void)setDestinations:(NSSet<BacktraceBaseDestination *> * _Nonnull)destinations;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
@property (nonatomic, copy) NSString * _Nullable deliveryOwner;
@property (nonatomic, strong) NSNumber * _Nullable deliveryStateRaw;
